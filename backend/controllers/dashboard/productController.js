...
class productController{
   ...
   add_product = async (req, res) => {
    ...
    form.parse(req, async (err, field, files) => {
        ...
        try {
            let allImageUrl = [];
            const imageArray = Array.isArray(images) ? images : [images]; // Ensure `images` is an array

            for (let i = 0; i < imageArray.length; i++) {
                console.log('Uploading Image:', imageArray[i].filepath); // Debug file path

                try {
                    const result = await cloudinary.uploader.upload(imageArray[i].filepath, { folder: 'products' });
                    allImageUrl.push(result.url);
                    console.log('Uploaded Image URL:', result.url); // Log uploaded URL
                } catch (uploadError) {
            ...
            console.log('Final Image URLs:', allImageUrl); // Debug final image URLs
            ...
            console.log('Error Adding Product:', error.message); // Log any error
